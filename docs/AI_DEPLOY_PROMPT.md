...
```text
请在我的 Windows 电脑上部署 cn-stock-mcp==0.2.1，并接入我当前使用的 MCP Host。

执行前先确认你具备本机 PowerShell、文件读写和 Host 配置权限。读取固定标签文档：
https://raw.githubusercontent.com/CodeGlimpse/cn-stock-mcp/v0.2.1/docs/AI_DEPLOY_WINDOWS.md

严格按该文档操作，并遵守以下边界：
1. 只安装 PyPI 的固定版本 0.2.1；先下载 wheel，再用同版本 GitHub Release 的 sha256sums.txt 核验 SHA256。不要安装 GitHub main、其他分支或未固定版本。
...
6. 验收版本必须为 0.2.1；默认 retail_v1_preview 必须正好显示 10 个工具；验证 MCP initialize、tools/list，并用 stock_quote 完成一次问答。
