...
- 核心代码继续免费按 MIT 开源；收费仅针对部署和代码运行售后，研发检查与客户交付验收分别记录。
- 补充 69 项 Windows 运行依赖的 109 份去重许可证/通知原文和完整性校验器，私人授权协议由经营者自行保管。
...
- 最终 SBOM 从经过安装验证的 Windows 运行环境生成；PyPI 使用独立上传目录，保留规范 release 文件集。
